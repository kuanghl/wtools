#!/bin/bash
# check json:   jq 'keys' /tmp/bench_16.json
#               jq '.completed' /tmp/bench_16.json
# check csv:    column -t -s, /tmp/benchmark_results.csv

RESULT_CSV="/tmp/benchmark_results.csv"
echo "concurrency,completed,failed,duration_s,qps_req_s,input_throughput_tps,output_throughput_tps,e2e_throughput_tps,mean_ttft_ms,p99_ttft_ms,mean_tpot_ms,p99_tpot_ms,mean_itl_ms,p99_itl_ms,mean_e2el_ms_est,p99_e2el_ms_est" > "$RESULT_CSV"
...
for CONC in 1 2 4 8 16; do
  ...
  if [ ! -f "$JSON_OUT" ]; then
    ...
    echo "$CONC,N/A,N/A,N/A,N/A,N/A,N/A,N/A,N/A,N/A,N/A,N/A,N/A,N/A,N/A,N/A" >> "$RESULT_CSV"
    continue
  fi
  ...
  # Output throughput (E2E average output token rate)
  ...
  echo "$CONC,$COMPLETED,$FAILED,$DURATION,$QPS,$INPUT_TPS,$OUTPUT_TPS,$E2E_TPS,$MEAN_TTFT,$P99_TTFT,$MEAN_TPOT,$P99_TPOT,$MEAN_ITL,$P99_ITL,$MEAN_E2EL,$P99_E2EL" >> "$RESULT_CSV"

  echo "Done: QPS=$QPS req/s | InTPS=$INPUT_TPS tok/s | OutTPS=$OUTPUT_TPS tok/s | E2ETPS=$E2E_TPS tok/s"
